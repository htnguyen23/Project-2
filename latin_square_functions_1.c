...
void Read_Latin_Square_File(const char *filename, 
	...
	/*  //trying to use fgets
	char *file_all = malloc(100 * sizeof(char)); 
	while (fgets(file_text, 1000, fp)) {  // fget = gets data from file (destination, how much, source)
		printf("%s\n", file_text);
		/* if (isalpha(*file_text) || isdigit(*file_text))
			count += 1; 
		if (*file_text != '\n' || *file_text != '\t' || *file_text != ' ' || *file_text != '\0') {
			count += 1;
			printf("count = %d\n", count);
		} 
	}
	...
	char file_text;
	char buffer[100]; 
	char* file_text_ptr = &file_text;
	size_t sizet = fread(buffer, sizeof(char), sizeof(buffer), fp);
	printf("sizet: %d\n, sizet");
	*/

	// trying to use fgetsc
	char ch;
	int ichar = 0;
	int newlineFlag = 0;
	int count = 0;
